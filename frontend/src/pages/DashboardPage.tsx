      ...
      <div className="max-w-5xl mx-auto px-6 mt-12 space-y-12">
         ...
         <section>
            ...
            {spaces.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {spaces.slice(0, 6).map(space => (
                  <Link 
                    key={space.id} 
                    to={`/space/${space.id}`}
                    className="flex flex-col p-5 bg-card border border-border rounded-[32px] hover:border-primary/20 hover:shadow-md transition-all text-left shadow-sm group aspect-square justify-between"
                  >
              ...
              <div className="bg-secondary/50 rounded-[40px] p-12 border border-dashed border-border text-center">
                 ...
                 <Button onClick={() => toast.info("Use the sidebar to create new spaces!")} variant="outline" className="mt-6 rounded-2xl font-bold text-sm h-10 px-6 border-border bg-background hover:bg-secondary">Add Space</Button>
              </div>
            )}
         </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
               {historyItems.slice(0, 3).map((item) => (
                ...
                {historyItems.length === 0 && (
                  <div className="col-span-full py-12 text-center bg-secondary/50 rounded-[40px] border border-dashed border-border text-center">
                     <div className="w-16 h-16 bg-card rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-sm border border-border">
                     ...
                     <Link to="/dashboard" className="mt-2 text-xs font-semibold text-muted-foreground/50 hover:text-foreground transition-colors">Start learning something new</Link>
                  </div>
                )}
            </div>
         </section>
