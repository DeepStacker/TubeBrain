...
class TranscriptEngine:
    ...
    def _check_whisper_available(cls) -> bool:
        """Check if any Whisper library is available (cached)."""
    ...
    async def extract(
        ...
        # Stage 4 & 5: Cloud transcription (Groq + Gemini in parallel for speed)
        await report(4, 6, "Transcribing with cloud AI...")
        logger.info(f"[{video_id}] Stage 4-5: Cloud transcription (parallel)")
        
        # Try cloud APIs in parallel
        cloud_result = await self._try_cloud_transcription_parallel(url, video_id)
        if cloud_result and cloud_result.word_count >= self.MIN_WORD_COUNT:
            logger.info(f"[{video_id}] ✓ Cloud transcription success ({cloud_result.word_count} words)")
            return cloud_result
